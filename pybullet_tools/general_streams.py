...
def process_motion_fluents(fluents, robot, verbose=False):
    ...
    for atom in sorted_fluents:
        ...
        if predicate in ['atpose', 'atrelpose']:
            o, p = args[:2]
            ## p may be a UniqueOptValue placeholder (no assign()) — skip if so
            if o not in ['@world'] and not isinstance(o, tuple) and hasattr(p, 'assign'):
                p.assign()
        elif predicate in ['atgrasp', 'atgrasphalf']:
            a, o, g = args
            ## g may be a UniqueOptValue placeholder (no get_attachment()) — skip if so
            if hasattr(g, 'get_attachment'):
