...
def get_ik_fn_old(problem, custom_limits={}, collisions=True, teleport=False,
    ...
    def fn(arm, obj, pose, grasp, base_conf, fluents=[]):
        ...
        if fluents:
            ## if any position/pose fluent is still an optimistic placeholder (UniqueOptValue),
            ## the world state is incomplete — return None so PDDLStream defers IK evaluation
            ## until the fluent (e.g. door-open position) is concretely grounded.
            ## without this, IK runs with door at its closed position → wrong trajectory.
            for atom in fluents:
                predicate = atom[0].lower()
                if predicate in ['atpose', 'atrelpose']:
                    p = atom[2]
                    if not hasattr(p, 'assign'):
                        return None
                elif predicate == 'atposition':
                    p = atom[2] if len(atom) > 2 else atom[1]
                    if not hasattr(p, 'assign'):
                        return None
